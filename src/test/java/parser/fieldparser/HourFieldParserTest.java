...
public class HourFieldParserTest {
    ...
    public void validInput_onlyRange_valueMatch() {
        ...
        for(int i = fieldParser.getMin(); i <= fieldParser.getMax(); i++)
            valueSet.add(i);
        assertEquals(valueSet, fieldParser.parseField("0-23"));
    }

    @Test
    public void validInput_onlyStep_valueMatch() {
        valueSet = new TreeSet<>(Arrays.asList(1, 11, 21));
        assertEquals(valueSet, fieldParser.parseField("1/10"));
    }

    @Test
    public void validInput_onlyStepWithAsterisk_valueMatch() {
        valueSet = new TreeSet<>(Arrays.asList(0, 20));
        assertEquals(valueSet, fieldParser.parseField("*/20"));
    }

    @Test
    public void validInput_onlyList_valueMatch() {
        valueSet = new TreeSet<>(Arrays.asList(0, 4, 23));
        assertEquals(valueSet, fieldParser.parseField("0,4,23"));
    }

    @Test
    public void validInput_listRangeAndStep_valueMatch() {
        valueSet = new TreeSet<>(Arrays.asList(0, 1, 2, 3, 4, 10, 11, 12, 20));
        assertEquals(valueSet, fieldParser.parseField("0,4,10-12,0/10,0-4,20/20"));
    }
